...
STYLE = (
    ...
    "default-sized page is never evidence that someone's history is short. "
...
_INSTRUCTIONS_BODY = (
    ...
    "Older days are sealed here and decryptable right now; you simply did not "
    "ask for them. Every `limit` defaults to a page size, not to a lifetime, and "
    "on the dense kinds it counts SAMPLES: 30 rows of hourly data is a day and a "
    "quarter, not a month. Re-read with a larger `limit`, or quote "
    "`coverage.oldest_available` as the real start of their history. Reporting "
